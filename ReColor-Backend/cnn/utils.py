...
Contains helper functions for color conversion, CPU optimization, K-means clustering, and common utilities.
"""
...
def setup_cpu() -> Dict[str, any]:
    """
    Set up CPU for TensorFlow and return device information.
    
    Returns:
        Dictionary containing CPU information and configuration
    """
    cpu_info = {
        'cpu_available': True,
        'cpu_cores': None,
    ...
    try:
        ...
        logging.info(f"✅ CPU setup complete: {cpu_info['cpu_cores']} cores available")
        logging.info(f"TensorFlow version: {cpu_info['tf_version']}")
        logging.info("Using CPU optimization for maximum performance")
            
    except Exception as e:
        logging.error(f"Error setting up CPU: {e}")
        # Continue anyway as CPU should always be available
