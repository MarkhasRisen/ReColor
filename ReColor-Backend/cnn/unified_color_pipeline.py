...
from error_handler import (
    error_context, AnomalyDetector, AnomalyType, AnomalyEvent
)
from kmeans_color_detector import KMeansColorDetector, get_kmeans_color_info
...
class UnifiedColorPipeline:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            enable_cpu_optimization: Enable CPU processing optimization
        """
        self.color_model = color_model
        self.cvd_simulator = cvd_simulator
        self.kmeans_clusters = kmeans_clusters
        self.cnn_patch_size = cnn_patch_size
        self.enable_cpu = enable_cpu_optimization
        ...
        self.logger.info(f"CPU optimization: {'enabled' if self.enable_cpu else 'disabled'}")
    ...
    def process_frame(self, 
        ...
        """
        Process a video frame through the complete pipeline with enhanced error handling.
        ...
        with error_context("unified_pipeline", self.logger, self.anomaly_detector):
            ...
            if frame is None:
                ...
                anomaly = AnomalyEvent(
                    ...
                    description='Input frame is None',
                    context={'frame_type': type(frame)}
                )
                self.anomaly_detector.log_anomaly(anomaly)
                return self._create_fallback_result(frame)
            ...
            if len(frame.shape) != 3 or frame.shape[2] != 3:
                ...
                anomaly = AnomalyEvent(
                    ...
                    severity='high',
                    component='unified_pipeline',
                    description=f'Invalid frame dimensions: {frame.shape}',
                    context={'expected_dims': 3, 'actual_shape': frame.shape}
                )
                self.anomaly_detector.log_anomaly(anomaly)
                return self._create_fallback_result(frame)
            ...
            h, w = frame.shape[:2]
            if h < 64 or w < 64:
                self.logger.warning(f"Very small frame size: {w}x{h}")
                anomaly = AnomalyEvent(
                    timestamp=time.time(),
                    anomaly_type=AnomalyType.INVALID_INPUT,
                    severity='medium',
                    component='unified_pipeline',
                    description=f'Small frame size: {w}x{h}',
                    context={'width': w, 'height': h, 'min_size': 64}
                )
                self.anomaly_detector.log_anomaly(anomaly)
            ...
            try:
                ...
                if color_families_result is None:
                    raise ValueError("K-means clustering failed")
                
                results['color_families'] = color_families_result
                kmeans_time = time.time() - kmeans_start
                self.performance_stats['kmeans_time'] = kmeans_time
                
                # Check K-means processing time
                if kmeans_time > 0.05:  # 50ms threshold
                    anomaly = AnomalyEvent(
                        timestamp=time.time(),
                        anomaly_type=AnomalyType.PROCESSING_TIMEOUT,
                        severity='low',
                        component='kmeans_clustering',
                        description=f'K-means processing time: {kmeans_time:.3f}s',
                        context={'processing_time': kmeans_time}
                    )
                    self.anomaly_detector.log_anomaly(anomaly)
                ...
                anomaly = AnomalyEvent(
                    ...
                    severity='high',
                    component='kmeans_clustering',
                    description=f'K-means failed: {str(e)}',
            ...
            try:
                ...
                if cnn_classifications is None:
                    raise ValueError("CNN classification failed")
                
                results['cnn_classifications'] = cnn_classifications
                cnn_time = time.time() - cnn_start
                self.performance_stats['cnn_time'] = cnn_time
                
                # Check CNN processing time
                if cnn_time > 0.1:  # 100ms threshold
                    anomaly = AnomalyEvent(
                        timestamp=time.time(),
                        anomaly_type=AnomalyType.PROCESSING_TIMEOUT,
                        severity='medium',
                        component='cnn_classification',
                        description=f'CNN processing time: {cnn_time:.3f}s',
                        context={'processing_time': cnn_time}
                    )
                    self.anomaly_detector.log_anomaly(anomaly)
                    results['anomalies_detected'].append(anomaly)
                ...
                anomaly = AnomalyEvent(
                    ...
                    severity='high',
                    component='cnn_classification',
                    description=f'CNN classification failed: {str(e)}',
                    context={'error_type': type(e).__name__}
                )
                self.anomaly_detector.log_anomaly(anomaly)
                results['anomalies_detected'].append(anomaly)
                # Continue with K-means results only
                cnn_classifications = {'patch_classifications': []}
                results['cnn_classifications'] = cnn_classifications
            ...
            try:
                ...
                self.logger.warning(f"Enhanced frame creation failed: {e}")
                results['processed_frame'] = frame.copy()  # Use original as fallback
            ...
            try:
                ...
                if cvd_type != CVDType.NORMAL:
                    ...
                    if daltonized_frame is None:
                        raise ValueError("Daltonization failed")
                    
                    results['daltonized_frame'] = daltonized_frame
                    dalton_time = time.time() - dalton_start
                    self.performance_stats['daltonization_time'] = dalton_time
                    
                    # Check daltonization processing time
                    if dalton_time > 0.05:  # 50ms threshold
                        anomaly = AnomalyEvent(
                            timestamp=time.time(),
                            anomaly_type=AnomalyType.PROCESSING_TIMEOUT,
                            severity='low',
                            component='daltonization',
                            description=f'Daltonization time: {dalton_time:.3f}s',
                            context={'processing_time': dalton_time}
                        )
                        self.anomaly_detector.log_anomaly(anomaly)
                        results['anomalies_detected'].append(anomaly)
                else:
                    results['daltonized_frame'] = results['processed_frame'].copy()
                    self.performance_stats['daltonization_time'] = 0.0
                    
            except Exception as e:
                self.logger.warning(f"Daltonization failed: {e}")
                anomaly = AnomalyEvent(
                    timestamp=time.time(),
                    anomaly_type=AnomalyType.PROCESSING_TIMEOUT,
                    severity='medium',
                    component='daltonization',
                    description=f'Daltonization failed: {str(e)}',
                    context={'error_type': type(e).__name__, 'cvd_type': cvd_type.value}
                )
                self.anomaly_detector.log_anomaly(anomaly)
                results['anomalies_detected'].append(anomaly)
                results['daltonized_frame'] = results['processed_frame'].copy()
            ...
            if total_time > 0.2:  # 200ms threshold for real-time processing
                anomaly = AnomalyEvent(
                    timestamp=time.time(),
                    anomaly_type=AnomalyType.PROCESSING_TIMEOUT,
                    severity='medium',
                    component='unified_pipeline',
                    description=f'Total pipeline time: {total_time:.3f}s',
                    context={'processing_time': total_time, 'fps': 1.0/total_time}
                )
                self.anomaly_detector.log_anomaly(anomaly)
                results['anomalies_detected'].append(anomaly)
            ...
            results['pipeline_stats'] = {
                ...
                'anomalies_count': len(results['anomalies_detected'])
            }
            
            return results
    
    def _create_fallback_result(self, frame: np.ndarray) -> Dict[str, Any]:
        """Create a safe fallback result when pipeline fails."""
        fallback_frame = frame.copy() if frame is not None else np.zeros((480, 640, 3), dtype=np.uint8)
        return {
            'original_frame': fallback_frame,
            'processed_frame': fallback_frame,
            'color_families': None,
            'cnn_classifications': None,
            'daltonized_frame': fallback_frame,
            'pipeline_stats': {
                'kmeans_time_ms': 0,
                'cnn_time_ms': 0,
                'daltonization_time_ms': 0,
                'total_time_ms': 0,
                'fps': 0,
                'frames_processed': 0,
                'anomalies_count': 1
