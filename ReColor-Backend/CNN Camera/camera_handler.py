...
class CameraHandler:
    ...
    def _add_color_info_overlay(self, frame: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            # Prepare text lines
            text_lines = [
                f"RGB: {info['dominant_rgb']}",
                f"HEX: {info['dominant_hex']}",
                f"Color: {info['rgb_color_name']}",
                f"AI: {info['predicted_color']} ({info['confidence']:.2f})"
            ]
    ...
    def _add_roi_indicator(self, frame: np.ndarray) -> np.ndarray:
        """Add ROI (region of interest) indicator to frame."""
        try:
            ...
            if self.show_side_by_side:
                ...
                cv2.rectangle(frame, (x1 + half_width, y1), (x2 + half_width, y2), (0, 255, 0), 2)
            else:
                # Single view
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
