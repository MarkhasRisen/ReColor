...
const bentoCard = (bg, onPress, icon, iconColor, title, desc, cardStyle) => (
  ...
  >
    ...
    <Text style={{ fontWeight: '700', fontSize: 15, color: '#1A1A2E', marginBottom: 4 }}>{title}</Text>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 32 }} showsVerticalScrollIndicator={false} style={{ backgroundColor: 'transparent' }}>
        <View style={{ marginTop: 10, marginBottom: 24 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <Text style={{ fontSize: 28, fontWeight: 'bold', flex: 1, marginRight: 10 }}>Welcome to ReColor</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 12, marginBottom: 12 }}>
          {bentoCard(
            ...
            'camera', '#9C27B0',
            'Color Enhancement',
            'Adaptive real-time color correction',
            { flex: 1 },
          )}
          {bentoCard(
            '#FCE4EC',
